          ...
          >
            {activeProfile ? (
              ...
              <div className="p-1 rounded-lg bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 shrink-0">
                <GemIcon iconName="sparkles" className="w-3.5 h-3.5" />
              </div>
            )}

            <span className="truncate max-w-[140px] sm:max-w-[240px]">
              {activeProfile ? activeProfile.name : 'Теа'}
            </span>
            ...
            <div className="absolute top-full left-0 mt-2 w-max min-w-[220px] max-w-sm bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-2xl shadow-xl p-2 z-50 animate-in fade-in zoom-in-95 duration-150">
              ...
              <div className="max-h-60 overflow-y-auto space-y-0.5 my-1">
                {profiles.map((p) => {
                  ...
                  const isBuiltIn = p.id === 'tea' || p.isBuiltIn
                  return (
                    <div
                      key={p.id}
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {isBuiltIn && (
                          <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 shrink-0">
                            Базовий
                          </span>
                        )}
                      </div>
