...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  // Класи для анімації виїзду меню та появи затемнення
  ...
  return (
    <>
      ...
      {/* Сама панель меню */}
      <aside className={`fixed top-0 left-0 h-full w-72 z-[70] bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 shadow-2xl transition-transform duration-300 ease-in-out flex flex-col ${sidebarClass}`}>
        <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800">
          <span className="font-semibold text-lg text-slate-800 dark:text-slate-200">Меню</span>
          <button 
            onClick={onClose}
